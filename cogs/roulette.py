...
# مسار مطلق: roulette.py خادم جوا cogs/، والصورة كاينة فجذر المشروع (جنب bg.png وprofile.png)
SHOP_IMAGE_PATH = Path(__file__).resolve().parent.parent / "yf_shop.jpg"
...
class LobbyView(discord.ui.View):
    ...
    async def shop(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if SHOP_IMAGE_PATH.exists():
            ...
            embed.title = "⚠️ صورة المتجر ناقصة"
            embed.description = f"خاصك تحط `yf_shop.jpg` فجذر المشروع (جنب `bg.png` و`main.py`).\nالمسار المتوقع: `{SHOP_IMAGE_PATH}`"
            await interaction.response.send_message(
                embed=embed,
                view=ShopView(g, interaction.user.id),
                ephemeral=True,
