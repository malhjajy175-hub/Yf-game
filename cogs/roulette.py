...
ITEMS = {
    "meteor":  {"name": "نيزك",   "price": 100, "emoji": "☄️", "type": "active"},
    "doctor":  {"name": "طبيب",   "price": 20,  "emoji": "💉", "type": "active"},
    "snipe":   {"name": "قنص",    "price": 20,  "emoji": "🎯", "type": "active"},
    "revive":  {"name": "انعاش",  "price": 20,  "emoji": "💫", "type": "active"},
    "block":   {"name": "منع",    "price": 10,  "emoji": "🚫", "type": "active"},
    "bomb":    {"name": "قنبلة",  "price": 50,  "emoji": "💣", "type": "active"},
    "shield":  {"name": "حماية",  "price": 15,  "emoji": "🛡️", "type": "defense"},
    "reflect": {"name": "مرتدة",  "price": 25,  "emoji": "🔄", "type": "defense"},
    "immune":  {"name": "مضاد",   "price": 30,  "emoji": "☣️", "type": "defense"},
}
...
class LobbyView(discord.ui.View):
    ...
    async def join(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message("✅ دخلت اللعبة!", ephemeral=True)
        await self.refresh()
    ...
    async def leave(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message("🚪 خرجت، وترجعت نقاط مشترياتك.", ephemeral=True)
        await self.refresh()
    ...
    async def shop(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if interaction.user.id not in g.players:
            return await interaction.response.send_message("خاصك تدخل اللعبة أولاً.", ephemeral=True)
        pts = await get_points(interaction.user.id)
        balance = pts["solo"] + pts["group"]
        await interaction.response.send_message(
            content=f"🛒 **متجر الروليت** — رصيدك: {balance} نقطة",
...
class ShopView(discord.ui.View):
    ...
    def _make_button(self, key: str, data: dict):
        ...
        async def callback(interaction: discord.Interaction):
            ...
            if not ok:
                return await interaction.response.send_message("❌ نقاطك ما كافياش.", ephemeral=True)
